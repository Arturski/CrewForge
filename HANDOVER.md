...
Commit history on `main` tells the phase-by-phase story (Phases 0–6 + follow-ups).
...
- **Batch + HITL don't mix**: a batch runs rows unattended, but a `human_input` task still **blocks** every row at its gate (gates block in all modes) — so a batched workflow with a HITL task hangs on row 0 until you answer it in the Run console (or Stop the batch). Don't batch a workflow that has human-input tasks. The batch driver is **sequential** by design (one row finishes before the next starts) — a hung row stalls the whole batch.
